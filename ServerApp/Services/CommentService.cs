...
public class CommentService : ICommentService
{
    ...
    {
        var comments = _DbContext.Comments
            .Where(x => x.AuthorId == userId && x.Status == CommentStatus.Approved)
            .Include(x => x.Liked)
            .Include(x => x.Disliked);
    ...
    public async Task<bool> RemoveDislikeAsync(long commentId, long userId)
    {
        Comment? comment = await _DbContext.Comments
            .Where(x => x.Status == CommentStatus.Approved)
            .Include(x => x.Liked)
        ...
        if (comment.Disliked.All(x => x.Id != userId))
            return false;
        comment.Disliked.Remove(user);
        comment.Dislikes = comment.Disliked.Count;
        await _DbContext.SaveChangesAsync();
    ...
    public async Task<bool> RemoveLikeAsync(long commentId, long userId)
    {
        Comment? comment = await _DbContext.Comments
            .Where(x => x.Status == CommentStatus.Approved)
            .Include(x => x.Liked)
